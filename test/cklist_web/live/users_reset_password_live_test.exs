defmodule CklistWeb.UsersResetPasswordLiveTest do
  ...
  describe "Reset password page" do
    ...
    test "renders errors for invalid data", %{conn: conn, token: token} do
      ...
      result =
        ...
        |> render_change(
          users: %{"password" => "secret12", "password_confirmation" => "secret123456"}
        )
  ...
  describe "Reset Password" do
    test "resets password once", %{conn: conn, token: token, users: users} do
      {:ok, lv, _html} = live(conn, ~p"/users/reset_password/#{token}")
